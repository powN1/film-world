...
const SpecificFilterModal = (props) => {
  ...
  const [inputValue, setInputValue] = useState("")
	...
	return (
			<div className={"w-screen md:w-[70%] h-screen flex flex-col gap-y-3 fixed top-0 right-0 bg-white z-30 overflow-y-scroll duration-100 " +(modalVisible ? "translate-x-0" : "translate-x-[100%]")}>
				...
				<div className="relative px-3">
					...
					<input
						type=""
            placeholder="Search..."
						className="w-full py-2 pl-8 border-b border-gray-400/50 outline-none focus:border-yellow-400"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
				...
				<div className="flex flex-wrap gap-2 px-3">
					{children.filter(item => item.toLowerCase().includes(inputValue.toLowerCase())).map((element, i) => (
						<button
							key={i}
							className={
				...
				<div className="flex justify-between mt-auto pb-4 px-3">
					<button className="py-3 w-1/3 text-gray-400 font-bold" onClick={() => {setCurrentFilter(null); setModalVisible(false);}}>
						Clear
					</button>
					<button
						className={
