...
export const fetchComments = async ({ skip = 0, type, mediaId, setParentCommentCountFunc, commentArray = null }) => {
  ...
  await axios
    .post(import.meta.env.VITE_SERVER_DOMAIN + "/api/get-media-comments", {
      type,
      mediaId,
      skip,
    })
...
const Comments = ({ type }) => {
  ...
  if (!context) {
    throw new Error("Comments component must be wrapped in a context provider (ArticleContext or ReviewContext).");
  }
  const {
    media,
    setMedia,
        ...
        <div className="flex flex-col w-full lg:w-2/3 gap-y-4">
          ...
          {commentsArr && commentsArr.length ? (
            commentsArr.map((comment, i) => {
              return (
                <CommentCard key={i} type={type} index={i} leftVal={comment.childrenLevel * 4} commentData={comment} />
              );
            })
          ) : (
