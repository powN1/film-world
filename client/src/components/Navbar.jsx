...
const Navbar = () => {
  ...
  let {
    userAuth,
    userAuth: { access_token, admin, profile_img, firstName, surname, username },
    setUserAuth,
  ...
  const handleMediaFetch = async () => {
    ...
    const allMedias = [...movies.data.movies, ...series.data.series, ...games.data.games, ...actors.data.actors];
  ...
  useEffect(() => {
    ...
    if (prevSearchModalVisibleRef.current !== undefined && prevSearchModalVisibleRef.current !== searchModalVisible) {
      handleSearchModal();
    }
    if (prevShowMobileMenuRef.current !== undefined && prevShowMobileMenuRef.current !== showMobileMenu) {
      handleMobileMenu();
    }
    if (prevLoginModalVisibleRef.current !== undefined && prevLoginModalVisibleRef.current !== loginModalVisible) {
      handleLoginModal();
    }
    ...
    if (modalInputValue && allMedias.length !== 0) {
      setFoundMedias(
  ...
  }, [navbarSize, searchModalVisible, modalInputValue, showMobileMenu, lastScrollY, loginModalVisible]);
  ...
  return (
    <>
      <div className="bg-black">
          ...
          >
            ...
            <div
              className={
                ...
                (navbarSize === "small" ? "w-[40px] cursor-pointer group" : "grow max-lg:grow-0")
              }
              onClick={() => {
                handleMediaFetch();
            ...
            >
              <input
                type="text"
                className={"h-full max-lg:w-0 " + (navbarSize === "small" ? "w-0" : "w-full lg:input-box")}
            ...
            {access_token ? (
              <div className="relative flex items-center gap-x-3 max-h-[40px]">
                <Link to={`/user/${username}`} className="rounded-full border border-gray-400 p-[1px] cursor-pointer">
                  <img src={profile_img} alt="user image" className="h-[40px] w-[40px] object-cover rounded-full" />
                </Link>
                <Link to={`/user/${username}`} className="hidden lg:block capitalize cursor-pointer">
                  {firstName} {surname}
                </Link>
                <div
                  ...
                  tabIndex={0}
                >
                  <IoChevronDown className={"duration-300 origin-center " + (showProfilePanel ? "rotate-180" : "")} />
                </div>
                {showProfilePanel ? (
                  <div className="absolute top-0 right-0 translate-y-[50%] translate-x-[35%] lg:translate-x-[0] -mt-3 bg-gray-100 text-gray-600 w-36 lg:w-80 flex flex-col [box-shadow:_1px_1px_6px_rgb(0_0_0_/_100%)]">
                    ...
                    <Link to="/settings" className="flex items-center gap-x-2 p-3 hover:bg-gray-200 duration-300">
              ...
              >
                ...
                />
                <p className={navbarSize === "small" ? "text-xs" : "max-lg:text-xs"}>Sign in</p>
              </button>
            )}
            ...
            >
              ...
              />
              <p className={navbarSize === "small" ? "text-xs" : "max-lg:text-xs"}>Menu</p>
            </button>

            {/* Menu categories */}
            <ul className={"list-none max-lg:hidden " + (navbarSize === "small" ? "-order-1 grow" : "w-full")}>
                      ...
                      >
                        <Link
                          ...
                          className={
                            ...
                            (navbarSize === "small" ? "text-xs py-5 px-2" : "py-3 px-5")
                          }
                        >
                          {item.title}
                          {item.submenu && (
                          ...
                          <ul className="absolute left-0 list-none group-hover:block hidden z-10">
                            {item.submenu.map((submenu, submenuIndex) => {
                              return (
                                ...
                                >
                                  <Link to={submenu.path} className="block py-2 px-7 font-bold hover:text-yellow-500">
                                    {submenu.title}
                                  </Link>
                                </li>
                              );
        ...
        {searchModalVisible ? (
          <div className="w-screen h-screen fixed top-0 left-0 bg-white z-30 flex justify-center overflow-y-auto">
            <div className="w-[90%] md:w-[55%] flex flex-col py-8 gap-y-8">
              {/* Search */}
              <div className="flex gap-x-4">
                <div className="relative flex justify-between w-full">
                  ...
                  {allMedias.length === 0 && (
                    <AiOutlineLoading3Quarters className="absolute left-[11%] top-1/2 translate-y-[-50%] text-lg text-yellow-400" />
                  )}
                  {modalInputValue ? (
                    <button
              ...
              {modalInputValue ? (
                <div className="flex flex-col">
                  <div className="grid grid-cols-1 md:grid-rows-1 md:grid-cols-6 gap-x-5">
                    {/* SLIDES */}
                    {foundMedias.slice(0, 6).map((media, i) => {
                      const mediaType =
                        media.itemType !== "person" && media.itemType ? `${media.itemType.slice(0, -1)}` : "person";
                      const mediaLink = media.title ? media.titleId : media.personal_info.nameId;
                      const mediaTitle = media.title ? media.title : media.personal_info.name;
                  ...
                  {foundMedias.length !== 0 ? (
                    <Link
                      to="/"
                      className="self-center py-2 px-14 my-6 border border-gray-200 font-bold hover:bg-black hover:text-white duration-300"
                    >
                      See more results
                    </Link>
                  ) : null}
                ...
                <div className="flex flex-col md:flex-row gap-x-5 gap-y-5">
                  <div className="w-full md:w-1/2 flex flex-col gap-y-4">
                    <div className="flex justify-between items-end">
                      <p className="text-xl tracking-tighter">Best movies</p>
                      <Link
                        to="/movies"
                        className="text-sm md:text-xs text-yellow-700 hover:text-yellow-400 duration-500"
                        onClick={() => setSearchModalVisible(false)}
                      >
                        See all
                      </Link>
                  ...
                  <div className="w-full md:w-1/2 flex flex-col gap-y-4">
                    <div className="flex justify-between items-end">
                      <p className="text-xl tracking-tighter">Best series</p>
                      <Link
                        to="/series"
        ...
        {showMobileMenu ? (
          <div className="flex flex-col w-screen h-screen fixed top-0 left-0 bg-white z-30 overflow-hidden">
            <div className="bg-gradient-to-r from-[#4D2F3B] from-10% via-[#1D2236] via-40% to-[#6B949A] px-3 py-4 flex flex-col gap-y-2">
              <div
                className={"flex items-center justify-between " + (access_token ? "pb-2 border-b border-gray-400" : "")}
              >
                {access_token ? (
                ...
                <button className="" onClick={handleMobileMenu}>
                  <IoMdClose className="text-white text-4xl" onClick={() => setShowMobileMenu((prevVal) => !prevVal)} />
                </button>
              </div>
              {!access_token && (
        ...
        <div
          className={
            ...
            (loginModalVisible ? "fixed bg-opacity-50 backdrop-blur-sm " : "invisible bg-opacity-0")
          }
        ></div>
