...
const RankingResultActor = (props) => {
	...
	return (
		<div
			className={
				"w-full flex flex-col py-5 gap-5 " +
				(index % 2 === 1 ? "bg-gray-200/25" : "")
			}
		>
			...
			<div className="flex flex-col gap-y-5">
				<div className="flex gap-x-5 ml-16">
					{roles.slice(0, 5).map((role, i) => (
						<Link
							key={i}
							className={ "relative min-w-[110px] w-[110px] min-h-[110px] h-[110px] cursor-pointer " + (currentRole === role ? "border-2 border-yellow-400" : "border border-gray-300")}
				...
				<div className="ml-16">
					...
					<span className="text-gray-400">
						in{" "} {currentRole.movie ? "movie" : currentRole.serie ? "serie " : null}{" "}
					</span>
					<span>
						{currentRole.movie ? currentRole.movie.title : currentRole.serie ? currentRole.serie.title : null}
					</span>
				</div>
			</div>
		</div>
